...
model_path = "Weight_1104_new_K/.pth"
...
def save_checkpoint(model, epoch):  # save model function
    model_out_path = 'Weight_1104_new_K' + '/' + "{}.pth".format(epoch)
...
if __name__ == "__main__":
    train_set = Dataset_Pro('./train_wv3.h5') # creat data for training
    ...
    #validation_data_loader = DataLoader(dataset=MyDataset(validationdata_path), num_workers=0, batch_size=batch_size, shuffle=True,
                                   #  pin_memory=True, drop_last=True)
    validate_set = Dataset_Pro('./valid_wv3.h5') # creat data for validation
