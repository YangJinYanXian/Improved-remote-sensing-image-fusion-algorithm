...
class panbranch(nn.Module):
    def __init__(self, conv=default_conv):
        ...
        self.conv_1 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_output1 = nn.Conv2d(in_channels=64, out_channels=8, kernel_size=3, stride=1, padding=1, bias=True)

        self.downscale = nn.MaxPool2d(kernel_size=2, stride=2)
        self.relu = nn.ReLU(inplace=True)
        self.conv_12 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_22 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_output2 = nn.Conv2d(in_channels=64, out_channels=8, kernel_size=3, stride=1, padding=1, bias=True)
#############################

        self.conv_13 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_23 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv_output3 = nn.Conv2d(in_channels=64, out_channels=8, kernel_size=3, stride=1, padding=1, bias=True)
        init_weights(self.conv_input, self.conv_1, self.conv_2, self.conv_output1, self.conv_output2, self.conv_output3,
                     self.downscale, self.conv_12, self.conv_22, self.conv_23, self.conv_13)
    ...
    def forward(self, pan):  # h*w*1
        ...
        feature_1_downd = self.downscale(torch.add(feature_3_down, feature_1_down))
        feature_2_downd = self.relu(self.conv_13(feature_1_downd))  # h*w*64
        feature_3_downd = self.conv_23(feature_2_downd)  # h*w*64
        detail_out_downd = self.relu(self.conv_output1(torch.add(feature_3_downd, feature_1_downd)))  # h*w*64

        return detail_out, detail_out_down, detail_out_downd
...
class tdnet(nn.Module):
    def __init__(self):
        ...
        self.subpixel_up3 = nn.PixelShuffle(upscale)
        self.mulnet = mscb()
        self.mulnet2 = mscb()
        self.mulnet3 = mscb()
        self.mrab = MRAB()
    ...
    def forward(self, x, y):  # x= ms; y =pan ; z=pan(downsample)
        ...
        # global rs, rs1
        num_block = 6
        detail_out, detail_out_down, detail_out_downd = self.pan_branch(y)

        #ms_up = self.conv_up(x)
        #ms_up = self.subpixel_up3(ms_up)
        ms_up = self.mrab3(x, detail_out_downd)
        ms_up_out = self.mulnet3(torch.cat([ms_up, detail_out_downd], dim=1))

        ms_up_1 = self.conv_up3(ms_up_out)
